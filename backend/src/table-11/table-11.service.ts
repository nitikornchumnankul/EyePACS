...
export class Table11Service {
    ...
    async createTable(eye_photo_id: string, createTableDto: CreateTableDto): Promise<Table11> {
        try {
            ...
            const table = this.table11Repository.create({ value, eye_photo: photo })
    ...
    async getTable(eye_photo_id: string): Promise<Table11> {
        try {
            const eye_photo = await this.eyePhotosService.getEyePhotoById(eye_photo_id)
            return await this.table11Repository.findOne({ where: { eye_photo } })
        } catch(e) {
